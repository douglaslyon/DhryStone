 ...
 */
//$Id: Linpack.java,v 1.1.1.1 2007/01/21 16:43:20 lyon Exp $
...
*/
        r.setResidn(resid / (n * norma * normx * r.getPrecision()));
        r.setResidn(r.getResidn() + 0.005); // for rounding
        r.setResidn((int) (r.getResidn() * 100));
        r.setResidn(r.getResidn() / 100);

        r.setTime(total);
        r.setTime(r.getTime() + 0.005); // for rounding
        r.setTime((int) (r.getTime() * 100));
        r.setTime(r.getTime() / 100);

        r.setMflops(ops / (1.0e6 * total));
        r.setMflops(r.getMflops() + 0.0005); // for rounding
        r.setMflops((int) (r.getMflops() * 1000));
        r.setMflops(r.getMflops() / 1000);
        return r;
    }
